...
func makeTestNameBuilder(perCallSep, schemaOverride, providerSep string, useSeparator bool) *nameBuilder {
	nb := &nameBuilder{
...
func TestResolveSeparator(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "schema override when no per-call",
			perCall:      "",
			schemaOvr:    ".",
			provider:     "-",
			useSeparator: true,
		...
		{
			name:         "provider separator when no overrides",
			perCall:      "",
			schemaOvr:    "",
			provider:     "-",
			useSeparator: true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			nb := makeTestNameBuilder(tt.perCall, tt.schemaOvr, tt.provider, tt.useSeparator)
			nb.resolveSeparator()
