...
- **Separator priority chain** (highest to lowest): per-call `settings.separator` > schema-level `separator` in JSON library (when `useSeparator=true`) > provider-level `separator` (when `useSeparator=true`) > empty string (when `useSeparator=false`). The schema-level separator lives in `JsonConfigurationSchema.Separator` (JSON field `separator,omitempty`) and is passed to `newNameBuilder()` via a side channel: `ProviderConfig.NamingSchemas` → function struct `provider` pointer → `schemaSeparatorOverride` argument
