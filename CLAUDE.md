...
- **`SchemaTypeAttributes()` must never gain a `separator` field** — all attributes listed there are required in inline HCL object literals; adding `separator` would break every existing inline schema config
- **Separator priority chain** (highest to lowest): per-call `settings.separator` > provider-level `separator` (when `useSeparator=true`) > empty string (when `useSeparator=false`)
- **Provider functions cannot access provider config in Terraform** — `Configure()` is not called before provider function `Run()` by design (terraform-plugin-framework#1093, closed won't-fix). Functions receive all needed data as explicit parameters (`configurations`, `name_type`, `settings`, `name`). Do not attempt to pass a provider pointer to function structs.
